...
def greedy_initialize_single_n(num_trees: int):
    ...
    - For each subsequent tree:
      - Start it at radius R on a random angle.
      - Move inwards until collision or center.
      - Back off slightly if collision.

    This is cheaper than the full Kaggle baseline but good enough
    ...
    for _ in range(1, num_trees):
        ...
        placed_polys = placed_polygons(trees)
        ...
        while radius >= 0:
            ...
            # collision check vs all existing
        ...
        # if collision, back off slightly
        if collision_found:
            radius += 0.2
            cx = radius * vx
            cy = radius * vy
            ...
            # no collision at center, just drop at origin
...
def compact_layout(trees):
    """
    Simple "compaction": shift all trees so the bounding box touches near (0, 0).

    This is a weak proxy for a real compactor but tends to help after random moves.
    """
    if not trees:
        return
...
def simulated_annealing_for_n(n: int):
    ...
    for step in range(max_iter):
        ...
        # Propose move
        dx = random_small_delta(INIT_STEP_XY)
        dy = random_small_delta(INIT_STEP_XY)
        dtheta = random_small_delta(INIT_STEP_ANGLE)
        ...
        # Optional micro-compaction (light)
        # Here we just re-anchor near origin every 200 steps
        if step % 200 == 0:
            compact_layout(current)
...
def main():
    ...
    for n in range(1, MAX_N + 1):
        ...
        layouts[n] = best_trees
        total_cost += best_cost
        print(f"Finished n={n}, best_cost={best_cost:.6f}")
